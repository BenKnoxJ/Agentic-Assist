...
### 4. Live use, first hour — one workflow gap, one silent failure found
- **The system passed its own sentence test on real work**: asked about the Joseph Rowntree onboarding, it search-found Amy's 6 Aug go-ahead (unanswered), reconstructed the commercial timeline from mail, cross-referenced five Jira tickets, flagged the stalled subtasks and an unanswered @mention, and — correctly — declared what it had no tools to check (calendar, Confluence).
- **Gap found by use (first day, as §11 predicted):** "find the email from Amy" is a search problem; `list_recent_mail` is a 25-message window. Built `search_mail` (Graph `$search`, KQL) same session, TDD: client method, tool, prompt update. Live: `from:amy` → 10 hits spanning a month. Learned live: Jira's `/search/jql` rejects *unbounded* queries — tool description now teaches the restriction so the agent doesn't learn by failing.
- **Silent failure (the §9.1 characteristic one):** the service had **never traced to LangSmith**. systemd applies `EnvironmentFile` *after* `Environment=`, so the unit's `LANGSMITH_TRACING=true` lost to `.env`'s `false` on every boot since the unit existed; the Aug-4 "1 root/turn" measurement was a dev shell. Found only because T8's span measurement queried for today's traces and got zero. Fix: `.env` owns the flag, unit sets nothing, warning comment in the unit. Verified after: probe turn → `LangGraph → megumi → claude-agent-sdk` child span with cost_usd/num_turns/session_id. §13 item 8 answered; §9.2 corrected.
- **Housekeeping:** the pre-Session-4 test contamination left `session_id="stub-session"` checkpointed on the `/chat` default thread — the SDK now hard-rejects it (`--resume` wants a UUID). Cleared with `/new`. Turn-id contextvar propagation into tool handlers confirmed in journald; tool log lines carry counts and query shapes only.

### 5. Remaining live evidence, recorded
- **Containment probe:** "run `ls` for me" sent from Teams → refused. The threat model's built-in-denial control now has both halves: `test_runner_options.py` pins the wiring, the live refusal shows the deployed system honouring it.
- Client secret expiry recorded: ~June 2027 (VPS.md ledger).
- Owner direction, 7 Aug: full-estate access (calendar, SharePoint, OneDrive, Teams chats) and write capability (draft/send mail, attachments) confirmed as the destination — sequenced per §11.0's step-5 carry list and §8.2's backlog rule. OneDrive stays refused until `Files.Read.All` can be scoped.
