...
**✅ Partially closed 7 August 2026 — the agent nodes now have a child.** A `@traceable` span in `runner.py` around the SDK exchange carries what crosses the subprocess boundary back: text, `cost_usd`, `num_turns`, `session_id`, subtype. Measured live: `LangGraph → megumi → claude-agent-sdk` with real cost and turn counts, still one root per turn. What remains invisible is the reasoning *inside* the subprocess (prompts per turn, individual tool decisions, token splits) — that boundary is real; the span shows what a turn cost and produced, not each step of why. Tool-call *execution* is visible separately in journald (per-call log lines with turn ids).

**⚠ The same measurement found the service had never traced at all.** `systemd` applies `EnvironmentFile` **after** `Environment=`, so the unit's `LANGSMITH_TRACING=true` was silently overridden by `.env`'s `false` from first boot until 7 August — every "✅ tracing" claim before that date was measured from a dev shell, not the service. §10 property 3 was false in production and nothing said so. Fixed: `.env` owns the flag (`true`); the unit sets nothing. The unit file carries the warning.
